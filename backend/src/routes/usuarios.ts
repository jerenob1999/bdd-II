...
router.get("/", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const documentos = await Usuario.find({ activo: true });
    res.json(documentos);
  } catch (err) {
    next(err);
  }
...
router.get("/:id", async (req: Request<IdParams>, res: Response, next: NextFunction) => {
  try {
    ...
    const documento = await Usuario.findOne({ _id: req.params.id, activo: true });
    if (!documento) {
...
router.put("/:id", async (req: Request<IdParams>, res: Response, next: NextFunction) => {
  try {
    ...
    const documentoActualizado = await Usuario.findOneAndUpdate(
